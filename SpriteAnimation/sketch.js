...
let VanHelsingAnimation;

function preload() {
  spriteSheet = loadImage("assets/SpelunkyGuy.png");
  VanHelsingSheet = loadImage("assets/VanHelsing.png");
}
...
function setup() {
  ...
  /*VanHelsingSheet.loadPixels();
  let pixels = VanHelsingSheet.pixels;
  for (let i=0; i < pixels.length; i += 4) {
    if (pixels[i] === pixels[0] && pixels[i+1] === pixels[1] && pixels[i+2] === pixels[2]) {
      pixels[i+3] = 0;
    } 
  }
  VanHelsingSheet.updatePixels();
  */
  ...
  VanHelsingAnimation = new WalkingAnimation(VanHelsingSheet,80,80,50,175,9);
